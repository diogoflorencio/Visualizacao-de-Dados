...
db = client['news_2018']
# define colelctions
collections = ["carta_capital", "estadao", "folha", "oantagonista", "oglobo", "veja"]
...
for collection in collections:
    ...
    for article in db[collection].find({}):
        # Processing text
        word2lexicon(article["text"]) 
        processSentences(article["text"])
        # Compute news bias
        arg, sen, val, mod, pre = wmd_ratings(article["text"])
        # Insert bias in database
        db[collection].update_one({'_id' : article['_id']}, 
                         {'$set' : {'arg':arg,
                                    'sen':sen,
                                    'val':val,
                                    'mod':mod,
        ...
        index += 1
        print('Index: {0} - Progress: {1:.4f} %'.format(index, index / len_collection * 100), end='\r')